...
%        slow registration process and go directly into diffusion. You can
%        obtain tp from a previous run of the algorithm in output INFO.tp.
%
...
%     'verbose': (def false) Verbose output.
%
%     'MaxIter' (def 100) Maximum number of diffusion iterations.
%
...
%        percentage of pixels sampled by the registration algorithm.
%
%     'MaxIter': (def 5) Maximum number of registration iterations allowed
%        before the algorithm has to stop.
%
%     'MaxVal': (def 0) Maximum value of any B-spline coefficient. If all
%        coefficients are smaller than MaxVal, then the algorithm stops.
%
...
%        the images. Anything outside the binary masks will be ignored for
%        registration. This can be used to avoid artifacts and to speed up
%        the algorithm.
%
%     't0': (def {}) A list of structs with initial transforms in elastix
%        format. Each t0(I) is applied to image I before registration. This
%        can be used to provide a pre-alignment of the images, typically
%        rigid.
%
...
%        coefficients of the I-th transform at registration iteration J.
%
...
switch (transform)
    ...
    case 'BSplineTransform'
        ...
        % saved as temp files
        [optReg.mask, deleteMaskFiles, nVoxMask, maskType] = imToFiles(optReg.mask);
...
while (true)
        ...
        case 'regmatchedfilt'
            ...
            if (isempty(optReg.tp))
                ...
                for J = 1:N-1
                    ...
                    info.tp(J) = regmatchedfilt(im(J+1), im(J), ...
                        optReg.Angle);
    ...
    switch (transform)
        ...
        case 'BSplineTransform'
            ...
            optDiff.Transform = transform;
            optDiff.Epsilon = min(tp(1).Spacing) * 1e-3;
...
%             %%%%%%% DEBUG
%             save('/tmp/foo2.mat', 'ttot', 'RegIter', 'info', ...
%                 'infoTransfdiff', 'iterinfo', 'optDiff', 'optDiffList', ...
%                 'optElastix', 't', 'tref')
...
function [im, deleteFiles, nVox, imType] = imToFiles(im)
...
elseif (ischar(im)) % single filename
    ...
    % get metainformation
    if (nargout > 2) % user wants to know number of voxels ~= 0
