...
% As slice position is equal throughout time, T = 1;  
    ...
    scimat.axis(1).min = dcminfo(1).ImagePositionPatient(2) - scimat.axis(1).spacing/2; % min Y position of the left edge of the first voxel 
    scimat.axis(2).min = dcminfo(1).ImagePositionPatient(1) - scimat.axis(2).spacing/2; % min X position of the left edge of the first voxel 
    scimat.axis(3).min = dcminfo(1).ImagePositionPatient(3) - scimat.axis(3).spacing/2; % min Z position of the left edge of the first voxel
    if (ndims(scimat.data) > 3)
