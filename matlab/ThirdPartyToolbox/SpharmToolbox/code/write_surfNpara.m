...
function write_surfNpara(filename, confs)

load(filename);

if ~exist('faces', 'var') | ~exist('vertices', 'var') | ~exist('sph_verts', 'var')
    disp('One or more of faces, vertices, and spherical vertices are missing');
    return;
end
...
filename2 = sprintf('%s/%s_para.meta', confs.OutDirectory, na(1:end-4));
...
if type == 3
    disp('This function doesnot support any triangular mesh now');
elseif type == 4
