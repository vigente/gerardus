...
/* This program was written based on Florent Ségonne's original implementation available 
                 ...
                 const mxArray *prhs[])
{
  ...
  if (!mxIsDouble(prhs[0]) || mxIsComplex(prhs[0]) ||
      !(mrows == 1 && ncols == 28)) {
    mexErrMsgTxt("Input must be a row vector of 28 double.");
  }
  ...
  y = mxGetPr(plhs[0]);
	...
	/* check if a simple point */
	for(k=0;k<3;k++)
		for(j=0;j<3;j++)
			for(i=0;i<3;i++){
				nbh[i][j][k]=x[i+j*3+k*9];
/*				printf(" %d",nbh[i][j][k]); */
			}
	connectivity = x[27];
/*	printf("\nconnectivity=%d\n",connectivity); */
	*y = checkSimple(&nbh,connectivity);
